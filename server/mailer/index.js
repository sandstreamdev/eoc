...
const sendSignUpConfirmationLink = (req, resp) => {
  ...
  const info = `<p>It is nice to have you on board! Please just click the button below to confirm your account in ${PROJECT_NAME}!</p><p><b>Remember that confirmation button will be active only ${
...
const sendResetPasswordLink = (req, resp) => {
  ...
  const info = `<p>Reset your password by clicking reset button. If you have not requested password reset to your account, just ignore this message.</p><p><b>Remember that reset button will be active only ${
    hours === 1 ? `${hours} hour` : `${hours} hours`
  }.</b></p>`;
