$(document).ready(() => {
  ...
  $('#main-slider .slides li').each(function() {
    const background_src = $('.slider-bg').attr('src');
    $(this)
      .children('.slider-bg')
      .remove();
  ...
  const bottom_items = $('.bottom');
  let left_top_items = 0;
  let right_top_items = 0;
  let bottom_top_items = 0;

  $(window).load(() => {
    left_top_items = left_items.map(function() {
      return $(this).position().top;
    });
    right_top_items = right_items.map(function() {
      return $(this).position().top;
    });
    bottom_top_items = bottom_items.map(function() {
      return $(this).position().top;
    });

    if ($(window).scrollTop() > $(window).height()) {
      const scrollonscreen = $(window).scrollTop() + $(window).height();

      $(left_top_items).each(function(i) {
        if (scrollonscreen > this) $(left_items[i]).css({ left: 0 });
      });

      $(right_top_items).each(function(i) {
        if (scrollonscreen > this) $(right_items[i]).css({ right: 0 });
      });

      $(bottom_top_items).each(function(i) {
        if (scrollonscreen > this - 1100) $(bottom_items[i]).css({ bottom: 0 });
      });
    }
  });

  $('#main-slider .slides li').css(
    'padding-top',
    $(window).height() / 2 - $('.slide-content').height() / 2
  );
  ...
  $(window).scroll(() => {
    // Parralax
    const scaleBg = -$(window).scrollTop() / 4;
    ...
    if (iOS === false) {
      ...
      $('.content-block.contact').css(
        'background-position-y',
        scaleBg + $('.content-block.contact').offset().top - 120
      );
    }
  ...
  $('.portfolio-item-block').on('click', function() {
    const current_portfolio_item = this;
    ...
    if ($('body').width() >= 734) {
      if ($(this).is('.open')) {
        ...
        if ($(current_portfolio_item).attr('data-item') == 1) {
          $(current_portfolio_item).css(
            'margin-bottom',
            $('.details', current_portfolio_item).height() + 80
          );
          $(current_portfolio_item)
            .next('.portfolio-item-block')
            .css(
              'margin-bottom',
              $('.details', current_portfolio_item).height() + 80
            );
      ...
      $(current_portfolio_item).css(
        'margin-bottom',
        $('.details', current_portfolio_item).height() + 75
      );
      $('.details', current_portfolio_item).fadeIn();
    }
  });

  $('.close-details').on('click', e => {
    e.stopPropagation();
    e.preventDefault();
    $('.portfolio-item-block').css('margin-bottom', 0);
    $('.details').fadeOut();
  ...
  $('.navbar a').on('click', function() {
    if ($($(this).attr('href')).position()) {
      $('html, body').animate(
        { scrollTop: $($(this).attr('href')).position().top - 70 },
        500
      );
