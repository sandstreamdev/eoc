...
(function($) {
  $.fn.extend({
    easyResponsiveTabs(options) {
      ...
      this.each(function() {
        const $respTabs = $(this);
        const $respTabsList = $respTabs.find(
          `ul.resp-tabs-list.${options.tabidentify}`
        );
        const respTabsId = $respTabs.attr('id');
        $respTabs
        ...
        $respTabs.find('.resp-accordion').each(function() {
          $tabItemh2 = $(this);
          const $tabItem = $respTabs.find(`.resp-tab-item:eq(${itemCount})`);
          const $accItem = $respTabs.find(`.resp-accordion:eq(${itemCount})`);
        ...
        let tabNum = 0;
        if (hash != '') {
          const matches = hash.match(new RegExp(`${respTabsId}([0-9]+)`));
        ...
        $respTabs.find('[role=tab]').each(function() {
          ...
          $currentTab.click(function() {
            ...
            if (
              ...
              $respTabs
                .find(
                  `.resp-tab-content[aria-labelledby = ${$tabAria}].${
                    options.tabidentify
                  }`
                )
                .slideDown()
            ...
            if (historyApi) {
              ...
              const tabAriaParts = $tabAria.split('tab_item-');
              // var newHash = respTabsId + (parseInt($tabAria.substring(9), 10) + 1).toString();
              let newHash =
                respTabsId + (parseInt(tabAriaParts[1], 10) + 1).toString();
              if (currentHash != '') {
                const re = new RegExp(`${respTabsId}[0-9]+`);
